...
def test_cutlass3_gemm(dtype):
    ...
    #for dim in [128+1]:
        ...
        for i in range(100):
            ...
            B = torch.randn(4*dim, dim+0, dtype=dtype, device=DEFAULT_DEVICE)/math.sqrt(dim)
            #B = torch.randn(1, dim, dtype=dtype, device=DEFAULT_DEVICE)/math.sqrt(dim)
...
def test_gemm_4bit(dtype):
    ...
    for dim in [4096]:
        ...
        max_relerr = 0
        for i in range(1):
            #A = torch.rand(2, 4092, dtype=dtype, device=DEFAULT_DEVICE)
            #B = torch.rand(4*4092, 4092, dtype=dtype, device=DEFAULT_DEVICE)
            #A = torch.rand(1, 4096, dtype=dtype, device=DEFAULT_DEVICE)
            #B = torch.rand(4*4096, 4096, dtype=dtype, device=DEFAULT_DEVICE)
            A = torch.randn(1, dim+0, dtype=dtype, device=DEFAULT_DEVICE)
            B = torch.randn(4*dim, dim+0, dtype=dtype, device=DEFAULT_DEVICE)/math.sqrt(dim)
